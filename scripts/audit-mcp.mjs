...
const registeredToolCount = new Set([...serverSource.matchAll(/"(kurogi_[a-z0-9_]+)"/g)].map((match) => match[1])).size;
const reportedToolCount = Number(main.match(/const MCP_TOOL_COUNT = (\d+)/)?.[1]);
...
expect(main.includes("app.requestSingleInstanceLock()") && main.includes('app.on("second-instance"'), "Kurogi must keep a single GUI bridge owner.");
expect(main.includes('ipcMain.handle("mcp-info"'), "Electron must expose MCP setup information to the UI.");
expect(main.includes("const MCP_INTEGRATION_VERSION = 4") && main.includes("mcpVersion: MCP_INTEGRATION_VERSION"), "Electron MCP setup metadata must report V4.");
assert.equal(reportedToolCount, registeredToolCount, "MCP dialog tool count must match the registered server tools.");
...
expect(main.includes("stageProjectAssetsForRender") && main.includes("retryClosedBrowser") && main.includes("renderConcurrency"), "Renderer must stage large assets, limit concurrency, and retry closed Chromium sessions.");
...
expect(!dialog.includes("PROTOCOL V3") && !dialog.includes("Available in V3"), "MCP setup dialog must not advertise the retired V3 integration.");
expect(dialog.includes("info.mcpVersion") && dialog.includes("info.toolCount"), "MCP setup dialog must display live integration metadata.");
